...
%% TODO all tunnels
transform_xml(#xmlElement{name = 'ipgre-tunnel', content = C}) ->
    #ip_in_gre_tunnel{};
transform_xml(#xmlElement{name = 'vxlan-tunnel', content = C}) ->
    #vxlan_tunnel{};
transform_xml(#xmlElement{name = 'nvgre-tunnel', content = C}) ->
    #nvgre_tunnel{};
...
transform_xml(#xmlElement{name = 'properties', content = C}) ->
    #queue_properties{min_rate = get_from_child(integer, 'min-rate', C),
                      max_rate = get_from_child(integer, 'max-rate', C),
                      experimenters = lists:foldl(fun(#xmlElement{name = 'experimenter'} = E, Acc) ->
...
transform_xml(#xmlElement{name = 'flow-table', content = C}) ->
    #flow_table{resource_id = get_from_child(string, 'resource-id', C),
                ...
                write_setfield = transform_xml(get_child('write-setfields', C)),
                apply_setfield = transform_xml(get_child('apply-setfields', C)),
...
transform_xml(#xmlElement{name = 'logical-switch-resources', content = C}) ->
    lists:foldl(fun(#xmlElement{name = 'port'} = E, Acc) ->
...
-spec get_all_children(atom(), list(#xmlElement{})) -> list(any()).
get_all_children(Type, Children) ->
    lists:foldl(fun(#xmlElement{} = C, Acc) ->
